...
| `PROMPT_RETENTION_DAYS` | `7` | days to retain captured user prompts (last user message of each request only, never responses). `0` disables prompt capture entirely. |
...
- **Users** — create/rotate/disable/delete per-user bearer tokens, with
  per-user stats and a per-user prompt history (see below).
