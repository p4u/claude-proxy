...
conversation to the chosen credential (persisted across restarts); a long-lived
pin may move to a substantially better-provisioned Anthropic subscription after
an API header notice (`REBALANCE_SESSIONS`).
...
- **Sticky binding** — each Claude Code conversation pins to a single credential (the binding is persisted across restarts); a long-lived pin may move to a substantially better-provisioned Anthropic subscription after an API header notice; `REBALANCE_SESSIONS=0` disables it.
...
| `PROMPT_RETENTION_DAYS` | `7` | days to retain captured user prompts (last user message of each request only, never responses) and, for users with full capture enabled, whole conversations. `0` disables all capture. |
| `REBALANCE_SESSIONS` | `1` | Proactive long-session rebalance for direct Anthropic subscriptions, announced with an API header notice. `0` disables it; emergency failover is unchanged. |
| `CLIPROXY_API_KEY` | (generated by `make env`) | authenticates claude-proxy to the private CLIProxyAPI data API. |
| `CLIPROXY_MANAGEMENT_KEY` | (generated by `make env`) | authenticates the narrow server-side Codex account-management bridge. Never expose it to browsers. |
| `CLIPROXY_IMAGE` | pinned `v7.2.149` digest | CLIProxyAPI sidecar image. Upgrade deliberately and rerun translation/tool/streaming tests. |
...
### Proactive long-session rebalancing

Long-lived pins may move to a substantially better-provisioned Anthropic
subscription after an API header notice. On by default; disable with
`REBALANCE_SESSIONS=0` in `.env`, or `CLAUDE_PROXY_REBALANCE_SESSIONS=0` /
`--rebalance-sessions=false` for direct runs. Direct Anthropic subscriptions
only — the Codex sidecar and the pre-existing emergency failover (a pinned
credential that expires or gets revoked) are unchanged.

- **Dwell/cooldown** share the durable `bound_at` timestamp: a pin must be at
  least an hour old, with at most one proactive switch per conversation per
  hour. This survives restart. A separate, per-process gate allows at most one
  elective move per destination per 5 minutes and resets on restart. These
  are request-time eligibility gates, not a fixed-time scheduler.
- Source and destination must be active and resolve to the same endpoint.
  A destination **qualifies** when its token expiry is more than 5 minutes in
  the future, both 5h/7d usage ≤ 60%, its effective score is ≥ 4× the
  source's **and** (its unweighted room score is ≥ 2× **or** its weekly
  urgency factor is ≥ 4×) — the condition must hold for the two latest usage
  samples (≥ 5 min apart; latest ≤ 15 min old, older ≤ 30 min old; resets
  consistent/future when known).
- The first successful response on the old account announces the move with
  `X-Router-Rebalance: pending` and a generic `X-Router-Message`. A later
  eligible request revalidates the destination; once this conversation binding's
  earlier in-flight responses drain, the pin is switched and the header reads
  `switched`. `count_tokens` and background Haiku calls join the lease but
  never trigger a switch.
- A pending announcement expires after 15 min and is discarded on restart
  (re-announced). If the destination stops qualifying, the pending switch is
  cancelled.
- The migration may add latency: the switch waits (cancellably) for in-flight
  responses to drain — in-flight streams are never interrupted — and the prompt
  cache on the new account may need a cold rebuild. Requests with account-scoped
  files/containers/server tools are excluded from elective migration.
- In-flight tracking is local to one process, so a single serving instance is
...
model responses, trimmed to 4096 chars — so the Users page can show a per-user
prompt history. An hourly janitor purges rows older than the retention window;
set `PROMPT_RETENTION_DAYS=0` to disable capture entirely.

**Full conversation capture (per user, opt-in):** each user token carries a
`full_capture` flag, toggled from the Users page. It is **off** by default, and
`PROMPT_RETENTION_DAYS=0` overrides it (with retention disabled nothing is
captured for anyone). When on, the proxy additionally records the whole
conversation for that user — both sides — in the `conversation_message` table:
every user turn from the request's `messages` array (the Messages API is
stateless, so each request re-sends the full history) plus the assistant's
reply, reconstructed from the response stream. The Users page then offers a
message viewer and a `.md` export per conversation.

What is stored: the visible **text** of user and assistant turns, capped at
32768 characters per message. What is not: tool calls and tool results, images,
thinking blocks, system prompts, and anything from a request whose user has the
flag off. Rows share the `prompt_log` retention window and the same hourly
janitor.

Privacy caveat: this stores the literal content of conversations, including
whatever a user pastes into them — file contents, logs, credentials. Enable it
per user, with their knowledge, and keep the retention window short.
...
`make up` starts a pinned [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)
sidecar on a dedicated Docker network. Its API port is not published. OpenAI
...
The Codex OAuth client registers `http://localhost:1455/auth/callback`; OpenAI
does not permit replacing it with `TLS_DOMAIN`. Port 1455 is therefore bound to
host loopback only. In Web UI mode CLIProxyAPI forwards that callback to
`http://127.0.0.1:8317/codex/callback`. Because the main API port stays private,
copy the final failed loopback URL from the browser address bar into the modal.
The backend accepts either exact callback shape, verifies the OAuth state, and
passes the one-time result to the sidecar without storing it.

For a browser on another machine, forwarding port 1455 can still receive the
first callback:

```bash
ssh -L 1455:127.0.0.1:1455 <proxy-host>
```

The final `127.0.0.1:8317/codex/callback?...` redirect still needs the modal's
manual handoff unless that second loopback port is forwarded as well. Connected
accounts appear in the main Credentials table, where they can be disabled,
weighted, reauthorized, or deleted. Codex weights compete only with other
Codex accounts. GPT models are advertised to Claude Code as
`claude-gpt-*` and restored to `gpt-*` on the sidecar wire.
