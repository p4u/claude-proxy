...
| `UI_PASSWORD` | (generated by `make env`) | password for the web UI at `/ui`. Empty = UI disabled (`/ui/*` 404s). |
...
```
http://<HOST_BIND>:<HOST_PORT>/ui     # plain HTTP
https://<TLS_DOMAIN>/ui               # when Traefik/TLS is enabled — same port, no extra config
```

`make ui` prints the URL and whether `UI_PASSWORD` is currently set. Leaving
`UI_PASSWORD` empty disables the UI entirely (`/ui/*` 404s, nothing is
mounted).
