...
- **Per-user auth** — per-user bearer tokens enforced on `/v1/*`, `/api/*`, and `/admin/*`; managed via `users add|list|revoke`.
...
cd claude-proxy
make env                         # generates .env with current UID/GID
make pull                        # pulls the latest GHCR image
make up                          # starts the proxy
make health                      # → {"ok":true}
# Create the first user and save the printed token:
docker compose run --rm --no-deps claude-proxy users add alice --db /data/proxy.db
```
...
ANTHROPIC_AUTH_TOKEN=<token-from-users-add> \
claude
```
...
./bin/claude-proxy users add alice --db ./proxy.db   # prints token once
...
| `AGENT_AUTH_TOKEN` | (empty) | bearer token the agent bridge subprocess uses for its self-loop calls back to the proxy. Create one with `users add`. Required only when `ENABLE_API_BRIDGE=true`. |
...
All requests to `/v1/*`, `/api/*`, and `/admin/*` require
`Authorization: Bearer <token>`. `/health` is always public.

Tokens are managed with the `users` subcommand:

```bash
# Create a user — the token is printed exactly once.
./bin/claude-proxy users add alice --db ./proxy.db
# token: cp_<base64url>

# List users (never shows raw tokens — only a fingerprint).
./bin/claude-proxy users list --db ./proxy.db

# Revoke by name or numeric id.
./bin/claude-proxy users revoke alice --db ./proxy.db
```

Point Claude Code at the proxy using the token from `users add`:

```bash
ANTHROPIC_AUTH_TOKEN=cp_<token> ANTHROPIC_BASE_URL=http://<proxy>:8787 claude
```

Tokens are stored hashed (SHA-256); the raw token is never written to the
database or logs. `Bearer ` prefix matching is case-insensitive.

> [!IMPORTANT]
> Always create at least one user when `HOST_BIND=0.0.0.0` or the proxy is
> reachable beyond loopback. Without a user, all authenticated endpoints
> return 401, which effectively blocks access — but creating a user is the
> right way to grant it.
...
Pass your user token as the API key:

```python
client = anthropic.Anthropic(
    base_url="http://localhost:8787/api",
    api_key=os.environ["ANTHROPIC_AUTH_TOKEN"],  # cp_<token> from users add
)
```
...
> [!IMPORTANT]
> Always combine TLS with at least one user created via `users add`. HTTPS
> protects the *transport* but anyone who learns your domain name can still
> reach the listener — the per-user bearer token is what stops them from
> spending your subscription quota.
...
Setup
  ...
  make env                  Create/upgrade .env (UID/GID)
...
`--agent-auth-token` falls back to `AGENT_AUTH_TOKEN` env var (used only by the
agent bridge self-loop). All `creds` and `users` subcommands accept `--db`
(defaults to `./proxy.db`).
...
Loopback by default. Requires a valid user bearer token (same as `/v1/*`).
`/health` is always reachable so the docker healthcheck keeps working.
...
- **`HOST_BIND=127.0.0.1`** binds loopback only. If Claude Code runs on a
  different host, set `HOST_BIND=0.0.0.0` and ensure a user has been created
  with `users add` so that credentials are protected by bearer token auth.
