...
These routes are available only when the CLIProxyAPI sidecar URL and both
internal keys are configured. The browser never receives the management key or
...
- `GET /api/codex/oauth/status?state=...` → `{status:"wait"|"ok"|"error",error?}`.
- `POST /api/codex/oauth/callback` `{state,redirect_url}` → accepts only the
  Codex loopback callback shapes (`localhost:1455/auth/callback` or the Web UI
  forwarder's `127.0.0.1:8317/codex/callback`) with a matching state before
  completing a remote-browser flow.
- `POST /api/codex/oauth/cancel` `{state}` → cancels an abandoned flow.
- `POST /api/codex/accounts/status` `{name,auth_index?,disabled}` and
  `POST /api/codex/accounts/weight` `{name,weight}` update account routing;
  `POST /api/codex/accounts/delete` `{name}` removes the persisted sidecar auth.

OpenAI's registered Codex redirect URI is loopback-only and cannot be changed
...
- `CLIPROXY_API_KEY` and `CLIPROXY_MANAGEMENT_KEY` are generated by `make env`.
  Compose renders the data key into a tmpfs config, passes the management key by
  environment, mounts `data/cliproxy-auth`, and exposes only callback port 1455
  on `127.0.0.1`. CLIProxyAPI runs non-root with all capabilities dropped and a
